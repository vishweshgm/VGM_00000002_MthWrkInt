...
uint32_t I2CFailLogCounter;
uint32_t I2CBusyFalgIssueCounter;
uint32_t I2CErrorFlagIssueCounter;
 ...
 * @page is the number of the start page. Range from 0 to ATMEL_EEP_CHIP_PAGE_NUM-1
...
void EEPROM_Write_NUM (uint16_t page, uint16_t offset, float data,uint8_t eepchipaddress)
{

	float2Bytes(bytes_temp, data);

	EEPROM_Write(page, offset, bytes_temp, 4,(uint16_t) eepchipaddress);
}
...
float EEPROM_Read_NUM (uint16_t page, uint16_t offset,uint8_t eepchipaddress)
{
	...
	EEPROM_Read(page, offset, buffer, 4,(uint16_t) eepchipaddress);
