...
volatile unsigned long timeCounterSecond;
/* ----------------------- Static variables ---------------------------------*/
...
eMBPTimerStart( xMBPTimerHandle xTimerHdl )
{
    ...
    __HAL_TIM_ENABLE_IT(htimp, TIM_IT_UPDATE);
    __HAL_TIM_ENABLE(htimp);
    /*QPS*/
    MBP_EXIT_CRITICAL_SECTION(  );
    return eStatus;
}
...
eMBPTimerStop( xMBPTimerHandle xTimerHdl )
{
    ...
    __HAL_TIM_DISABLE_IT(htimp, TIM_IT_UPDATE);
    __HAL_TIM_DISABLE(htimp);
    MBP_EXIT_CRITICAL_SECTION(  );
    return eStatus;
}
...
{
	if (__HAL_TIM_GET_FLAG(htimp, TIM_FLAG_UPDATE) != RESET)
	{
		if (__HAL_TIM_GET_IT_SOURCE(htimp, TIM_IT_UPDATE) != RESET)
		{
			__HAL_TIM_CLEAR_IT(htimp, TIM_IT_UPDATE);
			timeCounterms++;
